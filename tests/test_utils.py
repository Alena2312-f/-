...
def test_currency_rates(mock_get):
    ...
    mock_response_usd.json.return_value = {"conversion_rates": {"RUB": 78.92}}
    mock_response_eur = Mock()
    mock_response_eur.json.return_value = {"conversion_rates": {"RUB": 90.34}}
    mock_get.side_effect = [mock_response_usd, mock_response_eur]

    result = currency_rates(['USD', 'EUR'])
    expected = [{"currency": "USD", "rate": 78.92}, {"currency": "EUR", "rate": 90.34}]
...
def test_fetch_stock_prices(mock_get):
    ...
    mock_get.return_value.json.return_value = {"Global Quote": {"05. price": 203.92}}

    list_stocks = ["AAPL"]

    result = get_price_stock(list_stocks)
    expected = [
        {"stock": "AAPL", "price": 203.92},
    ]
    assert result == expected
